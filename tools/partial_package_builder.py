...
parser = argparse.ArgumentParser("Partial Package builder", description="Easily package parts of the source code into a separate distribution.")
parser.add_argument("-toml-file", required=True, help="The pyproject.toml file to use for the patial package.")
...
class PartialPackageBuilder:
    ...
    def __init__(self, toml_file: str):
        ...
        self.package_dict = {p["include"]:p for p in
                             self.toml["tool"]["poetry"]["packages"]}
    ...
    def build_partial_package(self):
        with TemporaryDirectory() as tempdir:
            ...
            if readme is not None:
                copy2(readme, Path(tempdir, readme))
    ...
    def _include_path(self, path_string: str, tempdir: TemporaryDirectory):
        ...
        if pkg is None:
            raise ValueError(f"Include path '{path_string}' is not part of any included package.")
