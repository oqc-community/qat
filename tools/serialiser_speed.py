...
def get_test_builder(qubits=32, layers=200):
        ...
        for layer in range(layers):
            ...
            for two_q_layer in range(qubits-1):
                 builder.ECR(builder.model.get_qubit(two_q_layer), builder.model.get_qubit(two_q_layer+1))
...
if __name__ == "__main__":
    ...
    for layer in layers:
        ...
        json_times.append(serialisation_time(builder=builder, backend=SerialiserBackend.json))
        ujson_times.append(serialisation_time(builder=builder, backend=SerialiserBackend.ujson))
