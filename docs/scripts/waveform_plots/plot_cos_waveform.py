...
"""Cos waveform plot generator for documentation.

Run directly::

    poetry run python docs/scripts/waveform_plots/plot_cos_waveform.py
...
from qat.utils.waveform import Cos

configure_matplotlib()


def draw_figure(ax: "plt.Axes | None" = None) -> plt.Figure:
    """Draw an annotated dual-Cos waveform figure.
    ...
    waveform1 = Cos(frequency=frequency1, internal_phase=phase1).eval(time).real
    waveform2 = Cos(frequency=frequency2, internal_phase=phase2).eval(time).real
    ...
    ax.text(-43.0, -0.77, r"$\mathbf{C_1}$", color=color1, fontsize=label_size)
    ax.text(-43.0, 0.40, r"$\mathbf{C_2}$", color=color2, fontsize=label_size)
    ...
    ax.text(
        ...
        r"$\mathbf{w}$: width    $\mathbf{C_i}$: waveform",
        ha="center",
        va="top",
        fontsize=label_size,
    )

    T1_ns = 1.0 / frequency1 * 1e9
    T2_ns = 1.0 / frequency2 * 1e9
    ...
    peak_C2_ns = -phase2 / (2.0 * np.pi * frequency2) * 1e9

    ax.vlines(0.0, 0.0, 1.06, color="black", linewidth=0.8, linestyle="--")
    ax.vlines(peak_C2_ns, 1.0, 1.12, color=color2, linewidth=0.9, linestyle="--")
    phase_y = 1.12
    ax.annotate(
        "",
        xy=(0.0, phase_y),
        xytext=(peak_C2_ns, phase_y),
        arrowprops={"arrowstyle": "<->", "linewidth": 0.9, "color": color2},
    )
    ax.text(
        peak_C2_ns / 2.0,
...
if __name__ == "__main__":
    args = parse_args("Plot annotated Cos waveform examples.")
    fig, ax = plt.subplots(figsize=(8, 4))
    draw_figure(ax=ax)
