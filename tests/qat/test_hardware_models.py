...
class TestCachedProperties:
    def create_echo_hw_model(self, num_qubits):
        ...
        for dev in hw.quantum_devices.values():
            ...
            if isinstance(dev, Qubit):
                for pc in dev.pulse_channels.values():
                    pc.full_id()

        for chan in hw.physical_channels.values():
            chan.full_id()

        for chan in hw.pulse_channels.values():
            chan.full_id()
        return hw
    ...
    def test_quantum_device_error(self):
        hw = self.create_echo_hw_model(1)
        with pytest.raises(ValueError):
            hw.get_qubit(1).id = "test"

    def test_physical_channel_error(self):
        hw = self.create_echo_hw_model(1)
        with pytest.raises(ValueError):
            hw.physical_channels["CH1"].id = "test"

    def test_pulse_channel_error(self):
        hw = self.create_echo_hw_model(1)
        keys = list(hw.pulse_channels.keys())
        with pytest.raises(ValueError):
            hw.pulse_channels[keys[0]].id = "test"
