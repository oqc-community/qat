...
class TestCachedId:
    ...
    def test_swap_partial_ids(self, model):
        ...
        hashs_before = [
            model.pulse_channels[keys[0]].__hash__(),
            model.pulse_channels[keys[1]].__hash__(),
        ]
        print(model.pulse_channels[keys[0]].full_id())
        print(model.pulse_channels[keys[1]].full_id())
        old_id = model.pulse_channels[keys[0]].id
        model.pulse_channels[keys[0]].id = model.pulse_channels[keys[1]].id
        model.pulse_channels[keys[1]].id = old_id
        hashs_after = [
            model.pulse_channels[keys[1]].__hash__(),
            model.pulse_channels[keys[0]].__hash__(),
        ]
    ...
    def test_change_physical_channels(self, model):
