...
def apply_setup_to_hardware(hw, qubit_indices: list, seed: SeedType | None = None):
...
def get_jagged_echo_hardware(
    ...
    if qubit_indices is None or len(qubit_indices) == 0:
        qubit_indices = random.Random(seed).sample(range(1, qubit_count + 5), qubit_count)
    qubit_indices.sort()
    return apply_setup_to_hardware(model, qubit_indices, seed=seed)
