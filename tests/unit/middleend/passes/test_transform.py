...
class TestPopulateWaveformSampleTime:
    ...
    def test_sampled_pulse_gains_sample_time(self):
        ...
        builder.pulse(target=qubit.drive_pulse_channel.uuid, waveform=wf)

        assert builder.instructions[0].duration == 0
        assert builder.instructions[0].waveform.sample_time is None

        ir = PopulateWaveformSampleTime(self.hw, self.taget_data).run(builder)
        duration = 100 * self.taget_data.QUBIT_DATA.sample_time
        assert ir.instructions[0].waveform.duration == duration
        assert ir.instructions[0].duration == duration
        assert (
            builder.instructions[0].waveform.sample_time
            == self.taget_data.QUBIT_DATA.sample_time
        )

    def test_sampled_waveform_on_acquire_filter_sample_time(self):
        builder = QuantumInstructionBuilder(hardware_model=self.hw)
        qubit = self.hw.qubits[0]
        samples = np.random.rand(100)
        wf = SampledWaveform(samples=samples)
        acq_duration = self.taget_data.RESONATOR_DATA.sample_time * 50
        builder.acquire(
            qubit,
            duration=acq_duration,
            filter=Pulse(
                duration=acq_duration,
                target=qubit.resonator.acquire_pulse_channel.uuid,
                waveform=wf,
            ),
        )
        assert builder.instructions[0].filter.duration == acq_duration
        assert builder.instructions[0].filter.waveform.sample_time is None

        ir = PopulateWaveformSampleTime(self.hw, self.taget_data).run(builder)

        duration = 100 * self.taget_data.RESONATOR_DATA.sample_time
        assert ir.instructions[0].filter.waveform.duration == duration
        assert ir.instructions[0].filter.duration == duration
        assert (
            ir.instructions[0].filter.waveform.sample_time
            == self.taget_data.RESONATOR_DATA.sample_time
        )

    def test_acquire_without_filter_remains_unchanged(self):
        builder = QuantumInstructionBuilder(hardware_model=self.hw)
        qubit = self.hw.qubits[0]
        acq_duration = self.taget_data.RESONATOR_DATA.sample_time * 50
        builder.acquire(qubit, duration=acq_duration)
        assert builder.instructions[0].duration == acq_duration

        ir = PopulateWaveformSampleTime(self.hw, self.taget_data).run(builder)

        assert ir.instructions[0].duration == acq_duration

    def test_aqcuire_with_waveform_remains_unchanged(self):
        builder = QuantumInstructionBuilder(hardware_model=self.hw)
        qubit = self.hw.qubits[0]
        acq_duration = 2e-6
        wf = GaussianWaveform(width=acq_duration)
        builder.acquire(
            qubit,
            duration=acq_duration,
            filter=Pulse(
        ...
        assert ir.instructions[0].duration == acq_duration
        assert not hasattr(builder.instructions[0].filter.waveform, "sample_time")

    def test_pulse_with_waveform_remains_unchanged(self):
        builder = QuantumInstructionBuilder(hardware_model=self.hw)
        qubit = self.hw.qubits[0]
        wf = GaussianWaveform(width=1e-6)
        builder.pulse(target=qubit.drive_pulse_channel.uuid, waveform=wf)
        assert builder.instructions[0].duration == 1e-6

        ir = PopulateWaveformSampleTime(self.hw, self.taget_data).run(builder)

        assert ir.instructions[0].duration == 1e-6
        assert not hasattr(builder.instructions[0].waveform, "sample_time")

    def test_does_noting_to_non_pulse_instruction(self):
        builder = QuantumInstructionBuilder(hardware_model=self.hw)
        qubit = self.hw.qubits[0]
        ...
        assert ir.instructions[0].duration == 1e-6
        assert ir.instructions[1].repeat_count == 1
        assert ir.instructions[2].phase == 1
...
class TestRepeatSanitisation:
    ...
    def test_repeat_count_from_config(self, compiler_config, use_default):
        ...
        ir = RepeatSanitisation(target_data).run(builder, compiler_config=compiler_config)
        assert isinstance(repeat := ir.instructions[-1], Repeat)
...
class TestInstructionLengthSanitisation:
    ...
    def test_custom_pulse_not_sanitised(self, seed, resonator):
        ...
        target_data = self.get_target_data(seed)
        if resonator:
            channel = self.hw.qubit_with_index(0).acquire_pulse_channel
            max_length = target_data.RESONATOR_DATA.pulse_duration_max
            sample_time = target_data.RESONATOR_DATA.sample_time
        else:
            channel = self.hw.qubit_with_index(0).drive_pulse_channel
        ...
        InstructionLengthSanitisation(self.hw, target_data).run(builder)
        assert len(builder.instructions) == 1
        assert np.allclose(builder.instructions[0].waveform.samples, samples)
    ...
    def test_custom_pulse_sanitised(self, seed, resonator):
        ...
        for i, instr in enumerate(builder.instructions[:2]):
            assert len(instr.waveform.samples) == max_samples
            assert np.allclose(
                instr.waveform.samples, samples[i * max_samples : (i + 1) * max_samples]
            )
        assert len(builder.instructions[-1].waveform.samples) == excess_samples
        assert np.allclose(
            builder.instructions[-1].waveform.samples, samples[-excess_samples:]
        )
...
class TestResetTransformation:
    ...
    @pytest.fixture(scope="class", params=[ResetTransformation, ResetsToDelays])
    def passive_reset_pass(self, request):
        return request.param

    @staticmethod
    def _get_active_results_managers(channel_to_qubit: dict[PulseChannel, Qubit]):
        res = ActivePulseChannelResults()
        for ch, qubit in channel_to_qubit.items():
            res.add_target(ch, qubit)
        res_mgr = ResultManager()
        res_mgr.add(res)
        return res_mgr
    ...
    def test_qubit_passive_reset(
        ...
        res_mgr = self._get_active_results_managers({qubit.drive_pulse_channel: qubit})

        before = builder.number_of_instructions
        builder = passive_reset_pass(model, target_data).run(builder, res_mgr)
    ...
    def test_qubit_passive_reset_with_compiler_config_reset_times(
        self,
        ...
        before = builder.number_of_instructions
        builder = passive_reset_pass(model, target_data).run(
    ...
    @pytest.mark.parametrize("passive_reset_time", [3.2e-06, 1e-03, 5.0])
    def test_pulse_channel_passive_reset(
        self, passive_reset_time: float, model, function_seed, passive_reset_pass
    ):
        qubit_data = QubitDescription.random(seed=function_seed).model_copy(
            update={"passive_reset_time": passive_reset_time}
        )
    ...
    def test_passive_reset_with_no_drive_channel(
        self, passive_reset_time, reset_chan, model, function_seed, passive_reset_pass
    ):
        qubit_data = QubitDescription.random(seed=function_seed).model_copy(
            update={"passive_reset_time": passive_reset_time}
        )
        ...
        builder.add(Reset(qubit_targets={qubit_idx}))

        res_mgr = self._get_active_results_managers(
            {qubit.acquire_pulse_channel: qubit, qubit.measure_pulse_channel: qubit}
        )
    ...
    def test_ddrop_reset_instructions(self, model):
        """DDrop reset should emit one Pulse on the qubit reset channel and one on the
        ...
        res_mgr = self._get_active_results_managers(
            {qubit.pulse_channels.reset: qubit, qubit.resonator.pulse_channels.reset: qubit}
        )
        compiler_config = CompilerConfig(driven_reset=True)
        builder = ResetTransformation(model, TargetData()).run(
            builder, res_mgr, compiler_config=compiler_config
        )

        assert len([i for i in builder.instructions if isinstance(i, Reset)]) == 0, (
            "Check all resets have been removed."
        )

        pulses = [i for i in builder.instructions if isinstance(i, Pulse)]
        assert len(pulses) == 2, "Should 2 pulses in total"
        assert len({pulse.target for pulse in pulses}) == 2, "Should have 2 unique targets"

        delays = [i for i in builder.instructions if isinstance(i, Delay)]
        assert len(delays) == 2, "Should have 2 delays on each reset channels"
        assert len({delay.target for delay in delays}) == 2, "Should have 2 unique targets"

    def test_ddrop_params(self, model):
        """Test if ddrop parameters are updated and added to the pulses."""
        delay, width, q_drive_rate, ro_amp = 1e-6, 12.2e-6, 0.23e6, 0.12
        model.qubit_with_index(0).pulse_channels.reset.delay = delay
        model.qubit_with_index(0).pulse_channels.reset.pulse.width = width
        model.qubit_with_index(0).pulse_channels.reset.pulse.amp = q_drive_rate
        model.qubit_with_index(0).resonator.pulse_channels.reset.pulse.width = width
        model.qubit_with_index(0).resonator.pulse_channels.reset.pulse.amp = ro_amp
        ...
        qubit_pulse = [
            instr
            for instr in builder
            if isinstance(instr, Pulse) and qubit.pulse_channels.reset.uuid == instr.target
        ][0]
        assert qubit_pulse.waveform.width == width
        assert qubit_pulse.waveform.amp == q_drive_rate
        resonator_pulse = [
            instr
            for instr in builder
            if isinstance(instr, Pulse)
            and qubit.resonator.pulse_channels.reset.uuid == instr.target
        ][0]
        assert resonator_pulse.waveform.width == width
        assert resonator_pulse.waveform.amp == ro_amp
        delay_pulses = [
            instr
            for instr in builder
            if isinstance(instr, Delay) and qubit.pulse_channels.reset.uuid == instr.target
        ]
        for delay_pulse in delay_pulses:
            assert delay_pulse.duration == 1e-6
