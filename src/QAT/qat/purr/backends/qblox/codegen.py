...
class QbloxContext:
    def __init__(self, repeat: Repeat):
        ...
        self._frequency: float = (
            0.0  # Keeps record of the frequency shift on the target
        )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.sequence_builder.jlt(
            self._repeat_reg, self._repeat_count, self._repeat_label
        )
    ...
    def is_empty(self):
        return not (
            any(self.sequence_builder.waveforms)
            or any(self.sequence_builder.acquisitions)
        )
