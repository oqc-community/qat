...
def get_physical_buffers(hardware, instructions):
    position_map = hardware.create_duration_timeline(instructions)
...
def _test_setup(stark_shift_1=False):
    ...
    if stark_shift_1:
        freq_channel = qubit1.create_pulse_channel(ChannelType.freq_shift, amp=1.0, scale=1.0, frequency=8.5e9)
    ...
    buffers = get_physical_buffers(engine, qat_file.instructions)
    return buffers[physical_channel_1.id], buffers[physical_channel_2.id]

def test_no_stark_shift():
    qubit1_buffer, qubit2_buffer = _test_setup()

    assert np.isclose(qubit1_buffer, 1+0j).all()
    assert np.isclose(qubit2_buffer, 1+0j).all()

def test_stark_shift():
    qubit1_buffer, qubit2_buffer = _test_setup(stark_shift_1=True)
