...
class QuantumHardwareModelBuilder:
    ...
    def add_physical_baseband(self, frequency, **kwargs):
        self._add_component(PhysicalBaseband(frequency=frequency, **kwargs))
    ...
    def add_qubit(
        self, index, frequency, physical_channel, resonator, pulse_channels=None, **kwargs
    ):
        if pulse_channels is None:
            ...
            pulse_channels = {pc_drive.uuid: pc_drive}
    ...
    def add_resonator(self, frequency, physical_channel, pulse_channels=None, **kwargs):
        if pulse_channels is None:
            ...
            pulse_channels = {
                pc_measure.uuid: pc_measure,
                pc_acquire.uuid: pc_acquire,
            }

        resonator = Resonator(
            pulse_channels=pulse_channels, physical_channel=physical_channel, **kwargs
        )
        self._add_component(resonator)
