...
class ReadoutMitigation(NoExtraFieldsModel):
    """
    :param linear: Maps each individual qubit to its <0/1> state given the <0/1>
        probabilistic error that happens on the device. Note that linear assumes no
        cross-talk effects and considers each qubit independent.
        linear = {
            <qubit_number>: NDArray(2, 2)
        }
        The matrix element (i, j) of the NDArray maps the probability p(i|j).
    :param m3_available: Flag to enable a runtime mitigation strategy that builds up the
                        calibration it needs at runtime. For more info
    ...
    def validate_linear(cls, linear):
        for qubit_idx, qubit_map in linear.items():
            if not np.allclose(np.sum(qubit_map, axis=0), np.ones(qubit_map.shape[0])):
                raise ValueError(
                    f"Please provide a linear probability map for qubit {qubit_idx} where "
                    f"all probabilities p(i|{qubit_idx} sum to 1."
                )
