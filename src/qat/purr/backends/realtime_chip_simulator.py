        ...
        while not sweep_iterator.is_finished():
            ...
            for device in self.model.quantum_devices.values():
                ...
                if isinstance(device, Qubit):
                    ...
                    bb = self.model.basebands[baseband.id]
                    LO = bb.frequency
                    ...
                    dt = self.model.physical_channels[channel.id].sample_time
                    buf = resonator_buffers[channel.id]
                    if len(buf) < 2:
                        continue
                    self.channel_pulse_data[channel.id] = spline_time(dt, self.sim_t, buf)
            ...
            for channel, aqs in aq_map.items():
                for aq in aqs:
                    response = response_buffers[aq.physical_channel.id][
