...
from qat.purr.backends.codegen_base import DfsTraversal
...
class NewQbloxContext:
    ...
    def enter_sweep(inst: Sweep, contexts: Dict):
        ...
        for context in contexts.values():
            ...
            bound = context.attr_mgr.get_attribute(name, "bounds")
    ...
    def exit_sweep(inst: Sweep, contexts: Dict):
        ...
        for context in contexts.values():
            ...
            context.sequence_builder.jlt(register, bound.end, label)
...
class PreCodegenResult:
    contexts: Dict[PulseChannel, NewQbloxContext] = None
...
class PreCodegenPass(AnalysisPass):
    ...
    def run(self, builder: InstructionBuilder, res_mgr: ResultManager, *args, **kwargs):
        ...
        iter_bounds = binding_result.iter_bounds_map
        ...
        for i, inst in enumerate(builder.instructions):
            if isinstance(inst, Sweep):
                ...
                for t, context in contexts.items():
                    ...
                    context.attr_mgr.set_attribute(name, "bounds", iter_bounds[t][name])
                    context.attr_mgr.set_attribute(inst, "bounds", iter_bounds[t][name])
...
class NewQbloxEmitter(InvokerMixin):
    ...
    def emit_packages(
        ...
        cfg_walker = QbloxCFGWalker(res_mgr)
        cfg_walker.walk()
        return cfg_walker.create_packages()


class QbloxCFGWalker(DfsTraversal):
    def __init__(self, res_mgr: ResultManager):
        super().__init__()
        self.cfg_result: CFGResult = res_mgr.lookup_by_type(CFGResult)
        self.precodegen_result: PreCodegenResult = res_mgr.lookup_by_type(PreCodegenResult)

    def enter(self, block):
        iterator = block.iterator()
        while (inst := next(iterator, None)) is not None:
            if isinstance(inst, Repeat):
                NewQbloxContext.enter_repeat(inst, self.precodegen_result.contexts)
            elif isinstance(inst, Sweep):
                NewQbloxContext.enter_sweep(inst, self.precodegen_result.contexts)
            elif isinstance(inst, QuantumInstruction):
                if isinstance(inst, PostProcessing):
                    continue
                elif isinstance(inst, Synchronize):
                    NewQbloxContext.synchronize(inst, self.precodegen_result.contexts)
                    continue
                elif isinstance(inst, PhaseReset):
                    NewQbloxContext.reset_phase(inst, self.precodegen_result.contexts)
                    continue

                for target in inst.quantum_targets:
                    context = self.precodegen_result.contexts[target]
    ...
    def exit(self, block):
        ...
        while (inst := next(iterator, None)) is not None:
            if isinstance(inst, Repeat):
                NewQbloxContext.exit_repeat(inst, self.precodegen_result.contexts)
            elif isinstance(inst, Sweep):
                NewQbloxContext.exit_sweep(inst, self.precodegen_result.contexts)

    def walk(self):
        # TODO - run as visit to the entry block
        NewQbloxContext.prologue(self.precodegen_result.contexts)
        self.run(self.cfg_result.cfg)
        # TODO - run as visit to the exit block
        NewQbloxContext.epilogue(self.precodegen_result.contexts)

        # Remove empty contexts
        self.precodegen_result.contexts = {
            t: c for t, c in self.precodegen_result.contexts.items() if not c.is_empty()
        }

    def create_packages(self):
        return [
            context.create_package(target)
            for target, context in self.precodegen_result.contexts.items()
        ]
