    ...
    for instruction in instructions:
        ...
        qubit = next(
            (
                ...
                if qubit.get_acquire_channel().id == instruction.channel.id
            ),
            None,
        )
        if qubit is None:
...
class QuantumHardwareModel(HardwareModel, Calibratable):
    ...
    def add_quantum_device(self, *devices: QuantumDevice):
        for device in devices:
            existing_dev = self.quantum_devices.get(device.id, None)
            ...
            self.quantum_devices[device.id] = device
    ...
    def add_pulse_channel(self, *pulse_channels: PulseChannel):
        for pulse_channel in pulse_channels:
            existing_channel = self.pulse_channels.get(pulse_channel.id, None)
            if existing_channel is not None:
                ...
                raise KeyError(
                    f"Pulse channel with id '{pulse_channel.id}' already exists."
                )

            self.pulse_channels[pulse_channel.id] = pulse_channel
    ...
    def add_physical_channel(self, *physical_channels: PhysicalChannel):
        for physical_channel in physical_channels:
            existing_channel = self.physical_channels.get(physical_channel.id, None)
            if existing_channel is not None:
                # If we're the same instance just don't throw.
                ...
                raise KeyError(
                    f"Physical channel with id '{physical_channel.id}' already exists."
                )

            self.physical_channels[physical_channel.id] = physical_channel
    ...
    def add_physical_baseband(self, *basebands: PhysicalBaseband):
        for baseband in basebands:
            existing_baseband = self.basebands.get(baseband.id, None)
            if existing_baseband is not None:
                # If we're the same instance just don't throw.
                if existing_baseband is baseband:
                    continue

                raise KeyError(f"Baseband with id '{baseband.id}' already exists.")

            self.basebands[baseband.id] = baseband
    ...
    def _resolve_qb_pulse_channel(self, chanbit: Union[Qubit, PulseChannel]):
        if isinstance(chanbit, Qubit):
            ...
            quantum_devices = self.get_devices_from_pulse_channel(chanbit.id)
            primary_devices = [
                device
            ...
            if len(primary_devices) > 1:
                raise NotImplementedError(
                    f"Too many devices use the channel with id {chanbit.id} as "
