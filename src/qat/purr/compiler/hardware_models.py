...
class QuantumHardwareModel(HardwareModel, Calibratable):
    ...
    def export_pydantic(self):
        ...
        for qubit in self.qubits:
            ...
            new_phys_bb_q = device.PhysicalBaseband(
            ...
            new_phys_ch_r = device.PhysicalChannel(
            ...
            # Resonator
            measure_pulse_channel = qubit.measure_device.get_pulse_channel(
                ChannelType.measure
            )
            new_measure_pulse_channel = device.MeasurePulseChannel(
            ...
            acquire_pulse_channel = qubit.measure_device.get_pulse_channel(
                ChannelType.acquire
            )
            new_acquire_pulse_channel = device.AcquirePulseChannel(
            ...
            new_res_pulse_channels = device.ResonatorPulseChannels(
            ...
            new_drive_pulse_channel = device.DrivePulseChannel(
            ...
            try:
                ...
                new_freqshift_pulse_channel = device.FreqShiftPulseChannel(
            ...
            try:
                secondstate_pulse_channel = qubit.get_pulse_channel(
                    ChannelType.second_state
                )
                new_secondstate_pulse_channel = device.SecondStatePulseChannel(
            ...
            for pulse_channel in qubit.pulse_channels.values():
                if (
                    ...
                    if pulse_channel.channel_type == ChannelType.cross_resonance:
                        new_cr_pulse_channel = device.CrossResonancePulseChannel(
                        ...
                        pulse_channel.channel_type
                        == ChannelType.cross_resonance_cancellation
                    ):
                        new_crc_pulse_channel = (
                            device.CrossResonanceCancellationPulseChannel(
                                auxiliary_qubit=aux_qubit,
                                frequency=pulse_channel.frequency,
                                bias=pulse_channel.bias,
                                scale=pulse_channel.scale,
            ...
            new_cross_resonance_pulse_channels = FrozenDict(
                new_cross_resonance_pulse_channels
            )
            ...
            new_qubit_pulse_channels = device.QubitPulseChannels(
