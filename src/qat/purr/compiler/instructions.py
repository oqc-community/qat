...
def _add_channels(
    ...
    return sorted(list(unique_targets), key=lambda x: x.id)
...
class Pulse(Waveform):
    ...
    def __repr__(self):
        return (
            f"pulse {self.channel.id},{self.shape.value},{self.amp},"
...
class Acquire(QuantumComponent, QuantumInstruction):
    ...
    def __repr__(self):
        ...
        return f"acquire {self.channel.id},{self.time}{mode}{out_var}"
...
class DeviceUpdate(QuantumInstruction):
    ...
    def __repr__(self):
        return f"{self.target.id}.{self.attribute} = {str(self.value)}"
