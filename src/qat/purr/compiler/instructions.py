...
class PostProcessType(Enum):
    ...
    should be processed. It describes a type of classical post-processing:
        ...
        axis `ProcessAxis.TIME`.
        - `MEAN`: Takes an average over the given axis.
        - `LINEAR_MAP_COMPLEX_TO_REAL`: Maps the (complex) measured value onto a (real)
        z-projection using a supplied linear mapping.
        - `DISCRIMINATE`: Converts a z-projection to a classical bit by comparison with a
        supplied discrimination threshold.
    """
...
class AcquireMode(Enum):
    ...
    Note that different backends will only allow selected acquisition modes.
        ...
        the number of shots
        - `INTEGRATOR`: Down-conversion and integration of the acquisition data has occurred on
        the hardware so that a single data point is returned for each shot
    """
